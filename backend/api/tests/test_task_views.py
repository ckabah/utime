...
class TestTaskViews(TestCase):
    ...
    def test_today_list_get_url(self):
        response = self.client.get(f'{self.url}?frequency=today')
    ...
    def test_tomorow_list_get_url(self):
        response = self.client.get(f'{self.url}?frequency=tomorow')
        self.assertEqual(response.status_code, 200)
    ...
    def test_task_update(self):
        ...
        response = self.client.put(f'{self.url}1', data=data, content_type='application/json')
        self.assertEqual(response.status_code, 200)
