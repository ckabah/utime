        ...
        <div>
            ...
            <form onSubmit={updateTask} className="task-form flex flex-col gap-2">
                <div className='flex flex-col'>
                    ...
                    <input type="text" name="title" defaultValue={task.title} onChange={(e)=>handleChange(e)}/>
                </div>
                <div className='flex flex-col'>
                    <label htmlFor="start_date" >Start date</label>
                    <span>{errorMessage.start_date}</span>
                    <input type="datetime-local" name="start_date" value={`${task.start_date}`.slice(0, 19)} onChange={(e)=>handleChange(e)}/>
                </div>  
                <div className='flex flex-col'>
                    <label htmlFor="end_date">End ddate</label>
                    <span>{errorMessage.end_date}</span>
                    <input type="datetime-local" name="end_date" value={`${task.end_date}`.slice(0, 19)} onChange={(e)=>handleChange(e)}/>
                </div>
                <div className='flex flex-col' >
                    <label htmlFor="description">Description</label>
                    <span>{errorMessage.description}</span>
                    <textarea type="textarea" name="description" defaultValue={task.description} onChange={(e)=>handleChange(e)}/>
                </div>
                <div className='flex flex-col'>
                    <label htmlFor="status">Status</label>
                    <span>{errorMessage.status}</span>
                    <select name="status" defaultValue={task.status} onChange={(e)=>{handleChange(e);}} className='task-status'>
